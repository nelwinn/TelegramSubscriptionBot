...
async def main():
    ...
    async with bot:
        ...
        async def on_start(event):
            ...
            first = f'<a href="tg://user?id={event.sender.id}">{event.sender.first_name}</a>'
            await bot.send_message(event.sender.id, START_MESSAGE.format(first),
                                   buttons=[
                                       [Button.text("Get Started", resize=True)]],
                                   parse_mode='html')
        ...
        async def handle_new_user(event, existing_user=False):
            ...
            async with bot.conversation(PeerUser(event.sender.id), timeout=1500) as conv:
                ...
                success = await store_details({"user_id": event.sender.id, "fund_name": fund_name,
                                               ...
                                               "check_size": check_size, "name": name, "username": event.sender.username}, existing_user=existing_user)
                if success:
        ...
        async def admin_dashboard(event):
            ...
            buttons = CREATER_ONLY_BUTTONS + \
                DASHBOARD_BUTTONS if event.sender.username == BOT_OWNER else DASHBOARD_BUTTONS
        ...
        async def unsub(event):
            await remove_subscription(event.sender.id)
            await bot.send_message(event.sender.id, "You are now unsubscribed.")

        @bot.on(events.NewMessage(pattern="Generate backend CSV"))
        async def backend_report(event):
            try:
                await bot.send_file(event.sender.id, await generate_report())
            except Exception as e:
                print(str(e))
                await event.reply("Something went wrong, try again.")

        @bot.on(events.NewMessage(pattern="Add admin"))
        async def add_administrator(event):
            async with bot.conversation(event.sender.id, timeout=1000) as conv:
                await conv.send_message("Enter the username of the new admin: ")
                username = (await conv.get_response()).text
                await add_admin(username)
                await conv.send_message("admin added successfullY!")
